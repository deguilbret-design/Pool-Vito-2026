...
if 'Nom' in df_p.columns:
    ...
    with st.expander("📋 Sélections"):
        ...
        for u in users:
            st.markdown(f"#### **{u}**")
            ur = df_p[df_p['Nom'].astype(str).str.strip() == str(u).strip()]
            if not ur.empty:
                c1, c2 = st.columns(2)
                if cc: c1.markdown(f'<div class="bonus-card">🏆 <div><div class="bonus-label">Coupe</div><div class="bonus-value">{ur[cc].iloc[0]}</div></div></div>', unsafe_allow_html=True)
    ...
    with st.expander("📜 Règlement"):
        # BLOC 1: STRUCTURE
        st.markdown('<div class="rules-section"><div class="rules-title">🏒 1. Pointage Évolutif</div>', unsafe_allow_html=True)
        st.markdown('Points par victoire selon la ronde actuelle :', unsafe_allow_html=True)
        t = '<table><tr><th>Ronde</th><th>Victoire</th><th>Boni</th></tr>'
        t += '<tr><td>1/8</td><td>1 pt</td><td>+2</td></tr>'
        t += '<tr><td>1/4</td><td>2 pts</td><td>+2</td></tr>'
        t += '<tr><td>1/2</td><td>3 pts</td><td>+2</td></tr>'
        ...
        st.markdown('Points accordés si le nombre exact de matchs est prédit.', unsafe_allow_html=True)
        ...
        st.markdown('<p style="font-size:0.8rem; color:#64748b; margin-top:10px;"><i>*Exception Match 7 : Accordé même si ton équipe perd la série.</i></p></div>', unsafe_allow_html=True)

        # BLOC 3: PERFORMANCE GLOBALE
        ...
        st.markdown('• <b>Trophée MVP :</b> +10 pts si ton choix initial gagne le titre.', unsafe_allow_html=True)

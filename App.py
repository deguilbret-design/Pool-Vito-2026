...
# 2. STYLE CSS (TICKER RAPIDE ET TABLEAU COMPACT)
st.markdown("""<style>
.nhl-ticker-wrap { 
    width: 100%; overflow: hidden; background: #0b0f19; 
    border-bottom: 2px solid #1f77b4; margin: -50px -50px 30px -50px; padding: 10px 0; 
}
.ticker { display: inline-flex; width: max-content; animation: tk 9s linear infinite; }
...
if 'Nom' in df_p.columns:
    ...
    with st.expander("🔍 Analyse des points"):
        for u in users:
            st.subheader(f"Joueur : {u}"); st.write(pd.DataFrame(det[u]).to_html(index=False), unsafe_allow_html=True)
